...
#     def __init__(self, num_classes, loss_weight=[0.4, 0.6]):
#         super(CeDiceLoss, self).__init__()
#         self.bce = nn.BCEWithLogitsLoss() # If the output is logits (not softmax/sigmoid yet, then use this loss)
#         self.celoss = nn.CrossEntropyLoss()
#         self.diceloss = nDiceLoss(num_classes)
#         self.loss_weight = loss_weight

#     def forward(self, pred, target):
#         loss_bce = self.bce(pred[:, 0], (target == 0).float())
#         loss_ce = self.celoss(pred, target[:].long())
#         loss_dice = self.diceloss(pred, target, softmax=True)
#         loss = loss_bce + self.loss_weight[0] * loss_ce + self.loss_weight[1] * loss_dice
#         return loss
...
def cal_params_flops(model, size_h,size_w, logger):
    input = torch.randn(1, 3, size_h,size_w).cuda()
