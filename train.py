...
from models.threemunet import ThreeMUNet
import time
...
def main(config):
    ...
    if config.network == '3munet':
        model = ThreeMUNet(
            ...
            load_ckpt_path=None, #model_cfg['load_ckpt_path'],
            deep_supervision=config.deep_supervision,
            mask_update=config.mask_update,
            mfe_enable=config.mfe_enable,
            mcsa_enable=config.mcsa_enable
        )
        model.load_from()
    ...
    if args.mode=="train":
        if os.path.exists(resume_model):
            ...
            best_dice, best_hd95 =  checkpoint['best_dice'], checkpoint['best_hd95']
        ...
        for epoch in range(start_epoch, config.epochs + 1):
            ...
            loss, all_class_mean_dice, all_class_mean_hd95 = val_one_epoch(
                    val_loader,
                    model,
                    criterion,
                    epoch,
            ...
            if epoch >= config.start_epoch_best_model_saving and all_class_mean_dice >= best_dice:
                print(f"Saving model with best loss:{loss} and dice:{all_class_mean_dice}")
                torch.save(model.state_dict(), os.path.join(checkpoint_dir, 'best.pth'))
                min_loss = loss
                min_epoch = epoch
                best_dice = all_class_mean_dice
                best_hd95 = all_class_mean_hd95
            else:
                print(f"Current best - Loss:{loss}, dice:{best_dice}, hd95:{best_hd95}")
